...
from ..models import NhanVien, DonHang, LichSuNo, KhachHang
from ..schemas.nhan_vien import TaoNhanVien, CapNhatNhanVien, DangNhapPin
from ..utils import now_vn, period_start_vn, parse_duong_dan_anh, trang_thai_don_vi
...
def _serialize_nhan_vien(nv: NhanVien, so_don_giao: int = 0) -> dict:
    ...
    if getattr(nv, "don_hang_giao", None):
        ngay_giao = [o.delivered_at for o in nv.don_hang_giao if getattr(o, "delivered_at", None)]
        if ngay_giao:
    ...
    return {
        ...
        "created_at": nv.created_at.strftime("%Y-%m-%d %H:%M") if nv.created_at else "",
        "delivered_count": int(so_don_giao or 0),
        "last_delivered_at": lan_giao_cuoi.strftime("%Y-%m-%d %H:%M") if lan_giao_cuoi else "",
    }
...
def lay_danh_sach(db: Session = Depends(get_db)):
    ...
    return [_serialize_nhan_vien(nv, so_don_giao=thong_ke_giao.get(nv.id, 0)) for nv in nhan_vien]
...
def tao_nhan_vien(data: TaoNhanVien, db: Session = Depends(get_db)):
    ...
    nv = NhanVien(
        ...
        notes=data.notes.strip(), role=vai_tro, pin=pin, is_active=1,
    )
    db.add(nv)
    db.commit()
    db.refresh(nv)
...
def cap_nhat_nhan_vien(nv_id: int, data: CapNhatNhanVien, db: Session = Depends(get_db)):
    ...
    if data.pin is not None:
        ...
        ton_tai = db.query(NhanVien).filter(NhanVien.pin == pin_moi, NhanVien.id != nv_id).first()
        if ton_tai:
...
def lich_su_giao_hang(nv_id: int, q: str = "", days: int = 0, limit: int = 200, db: Session = Depends(get_db)):
    ...
    if bat_dau:
        query = query.filter(DonHang.delivered_at.isnot(None), DonHang.delivered_at >= bat_dau)
    don_hang = query.order_by(desc(DonHang.delivered_at), desc(DonHang.id)).limit(gioi_han).all()
...
def lich_su_hoat_dong(nv_id: int, q: str = "", days: int = 0, limit: int = 300, db: Session = Depends(get_db)):
    ...
    if bat_dau:
        qr_no = qr_no.filter(LichSuNo.created_at >= bat_dau)
    if tu_khoa:
        if tu_khoa.isdigit():
            qr_no = qr_no.filter((LichSuNo.id == int(tu_khoa)) | (KhachHang.name.ilike(f"%{tu_khoa}%")) | (LichSuNo.note.ilike(f"%{tu_khoa}%")))
        else:
    ...
    for don in danh_sach_don:
        ts = int(don.created_ts or 0) or int(don.created_at.timestamp() * 1000 if don.created_at else 0)
        hoat_dong.append({
            "type": "ORDER", "sort_ts": ts,
            "date": don.created_at.strftime("%Y-%m-%d %H:%M") if don.created_at else "",
    ...
    for row in danh_sach_no:
        log, ten_khach = row
        ts = int(log.created_ts or 0) or int(log.created_at.timestamp() * 1000 if log.created_at else 0)
