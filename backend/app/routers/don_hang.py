...
from ..models import BienThe, ChiTietDon, DonHang, KhachHang, KhuVuc, NhanVien
from ..schemas.don_hang import (
    CapNhatNgayDon,
    XacNhanLocalAnh,
    YeuCauGiaoHang,
    YeuCauNhanDon,
...
from ..utils import now_vn, now_vn_ts, parse_duong_dan_anh, trang_thai_don_vi
...
def _thu_muc_upload() -> str:
    thu_muc = settings.DELIVERY_UPLOAD_DIR
    if not thu_muc:
        thu_muc = "/tmp/delivery_proofs"
...
def _lay_khu_vuc_mac_dinh(db: Session):
    ...
    if kv:
        return kv.id
    dau = db.query(KhuVuc).order_by(KhuVuc.id).first()
    return dau.id if dau else None
...
def _serialize_don(don: DonHang) -> dict:
    ...
    return {
        "id": don.id,
        "created_at": don.created_at.strftime("%Y-%m-%d %H:%M") if don.created_at else "",
        ...
        "assigned_at": don.assigned_at.strftime("%Y-%m-%d %H:%M") if don.assigned_at else "",
        "delivered_by_id": don.delivered_by_id,
        "delivered_by_name": (don.nguoi_giao.name if getattr(don, "nguoi_giao", None) else ""),
        "delivered_at": don.delivered_at.strftime("%Y-%m-%d %H:%M") if don.delivered_at else "",
        ...
        try:
            if len(abs_paths) == 1:
                ...
                with open(abs_paths[0], "rb") as f:
                    r = req_lib.post(url, files={"photo": f}, data={"chat_id": chat_id, "caption": caption}, timeout=30)
                ...
                for i, path in enumerate(abs_paths):
                    ...
                    media.append(item)
                try:
                    r = req_lib.post(url, files=files, data={"chat_id": chat_id, "media": json.dumps(media, ensure_ascii=False)}, timeout=60)
                finally:
                    for fh in opened:
                        try:
                            fh.close()
...
def lay_don_hang(page: int = 1, limit: int = 20, db: Session = Depends(get_db)):
    try:
        ...
        for don in don_hang:
            ...
            ket_qua.append({"id": don.id, "created_at": don.created_at.strftime("%Y-%m-%d %H:%M") if don.created_at else "", "customer_name": don.customer_name or "Khách lẻ", "total_amount": int(don.total_amount or 0), "total_qty": tong_sl, "picker_note": (don.picker_note or ""), "items": chi_tiet})
...
def cap_nhat_ngay_don(don_id: int, data: CapNhatNgayDon, db: Session = Depends(get_db)):
    ...
    try:
        ...
        don.created_at, don.created_ts = ngay_moi, int(ngay_moi.timestamp() * 1000)
        db.commit()
    ...
    try:
        ...
        if ten_khach:
            ...
            if not khach:
                ...
                db.flush()
            khach.debt += tong
        don = DonHang(total_amount=tong, customer_name=khach.name if khach else "Khách lẻ", customer_id=khach.id if khach else None, is_draft=0, status="completed", created_by_employee_id=data.employee_id)
        don.created_ts = now_vn_ts()
        db.add(don)
        db.flush()
        for item in data.cart:
            db.add(ChiTietDon(order_id=don.id, product_name=item.product_name, variant_id=item.variant_id, variant_info=f"{item.color}-{item.size}", quantity=item.quantity, price=item.price))
        db.commit()
    ...
    try:
        ...
        if ten_khach:
            ...
            if not khach:
                ...
                db.add(khach)
                db.flush()
        don = DonHang(total_amount=tong, customer_name=khach.name if khach else "Khách lẻ", customer_id=khach.id if khach else None, is_draft=1, status="pending", created_by_employee_id=data.employee_id)
        don.created_ts = now_vn_ts()
        db.add(don)
        db.flush()
        for item in data.cart:
            db.add(ChiTietDon(order_id=don.id, product_name=item.product_name, variant_id=item.variant_id, variant_info=f"{item.color}-{item.size}", quantity=item.quantity, price=item.price))
        db.commit()
        return {"status": "success", "order_id": don.id, "message": "Đơn hàng đã gửi chờ staff tiếp nhận"}
    except Exception as e:
        db.rollback()
        raise HTTPException(status_code=500, detail=str(e))
    ...
    try:
        ...
        if ten_khach:
            ...
            if not khach:
                ...
                db.add(khach)
                db.flush()
        don = DonHang(total_amount=tong, customer_name=khach.name if khach else "Khách lẻ", customer_id=khach.id if khach else None, is_draft=1, status="approved")
        don.created_ts = now_vn_ts()
        db.add(don)
        db.flush()
        for item in data.cart:
            db.add(ChiTietDon(order_id=don.id, product_name=item.product_name, variant_id=item.variant_id, variant_info=f"{item.color}-{item.size}", quantity=item.quantity, price=item.price))
        db.commit()
        return {"status": "success", "order_id": don.id, "message": "Đơn desktop đã gửi picker, chờ nhận xử lý"}
    except Exception as e:
        db.rollback()
        raise HTTPException(status_code=500, detail=str(e))


@router.put("/orders/{don_id}")
def cap_nhat_don(don_id: int, data: YeuCauThanhToan, db: Session = Depends(get_db)):
    try:
        from sqlalchemy import func as sqla_func
        don_cu = db.query(DonHang).filter(DonHang.id == don_id).first()
        if not don_cu:
            raise HTTPException(status_code=404, detail="Không tìm thấy đơn hàng")
        if don_cu.status != "completed":
            raise HTTPException(status_code=400, detail="Chỉ có thể sửa đơn hàng đã hoàn thành")
        for ct in don_cu.chi_tiet:
            if ct.variant_id:
                bt = db.query(BienThe).filter(BienThe.id == ct.variant_id).first()
                if bt:
                    bt.stock += ct.quantity
        if don_cu.customer_id:
            kh = db.query(KhachHang).filter(KhachHang.id == don_cu.customer_id).first()
            if kh:
                kh.debt -= don_cu.total_amount
        db.query(ChiTietDon).filter(ChiTietDon.order_id == don_id).delete()
        tong_moi = sum(item.quantity * item.price for item in data.cart)
        for item in data.cart:
            bt = db.query(BienThe).filter(BienThe.id == item.variant_id).first()
            if not bt or bt.stock < item.quantity:
        ...
        if ten_khach:
            ...
            if not khach:
                ...
                db.flush()
            khach.debt += tong_moi
        don_cu.customer_name = ten_khach if ten_khach else "Khách lẻ"
        don_cu.customer_id = khach.id if khach else None
        don_cu.total_amount = tong_moi
        ngay_gio = now_vn()
        don_cu.created_at, don_cu.created_ts = ngay_gio, int(ngay_gio.timestamp() * 1000)
        for item in data.cart:
            db.add(ChiTietDon(order_id=don_cu.id, product_name=item.product_name, variant_id=item.variant_id, variant_info=f"{item.color}-{item.size}", quantity=item.quantity, price=item.price))
        db.commit()
        return {"status": "updated"}
    except Exception as e:
        db.rollback()
...
def lay_don_cho_duyet(db: Session = Depends(get_db)):
    try:
        ...
        ket_qua = []
        for don in don_hang:
            chi_tiet, tong_sl, co_thieu_hang = [], 0, False
            for ct in (don.chi_tiet or []):
                tong_sl += ct.quantity
...
def duyet_don(don_id: int, db: Session = Depends(get_db)):
    try:
        ...
        db.commit()
        return {"status": "success", "message": f"Đơn #{don_id} đã được duyệt, chờ picker nhận"}
...
def tu_choi_don(don_id: int, db: Session = Depends(get_db)):
    try:
        ...
        db.delete(don)
        db.commit()
        return {"status": "success", "message": f"Đơn #{don_id} đã bị từ chối và xóa"}
    except Exception as e:
        db.rollback()
        raise HTTPException(status_code=500, detail=str(e))
...
def huy_don(don_id: int, db: Session = Depends(get_db)):
    try:
        ...
        picker_id = don.assigned_picker_id
        db.query(ChiTietDon).filter(ChiTietDon.order_id == don_id).delete()
        db.delete(don)
        db.commit()
        return {"status": "success", "message": f"Đơn #{don_id} đã bị hủy", "assigned_picker_id": picker_id}
    except HTTPException:
        db.rollback()
        raise
    except Exception as e:
...
def lay_don_da_chap_nhan(db: Session = Depends(get_db)):
    """Backward-compat alias cho /orders/approved."""
    return lay_don_da_duyet(db=db)
...
def nhan_don(don_id: int, data: YeuCauNhanDon, db: Session = Depends(get_db)):
    try:
        ...
        if not picker or (picker.role or "").strip().lower() not in ("picker", "manager"):
            raise HTTPException(status_code=400, detail="Picker không hợp lệ")
        don.status, don.assigned_picker_id, don.assigned_at = "assigned", picker.id, now_vn()
        db.commit()
...
def _xac_nhan_giao_logic(don: DonHang, items: Optional[List[XacNhanGiaoItem]], db: Session, ghi_chu_picker: str = "") -> dict:
    """Core logic xác nhận giao: hoàn kho thiếu, ghi nhận nợ, đánh dấu completed."""
    ...
    tong_giao, thieu_hang, loi_kho = 0, [], []
        ...
        if sl_giao < sl_yc:
            ...
            if ct.variant_id:
                ...
                if bt:
                    ...
                    loi_kho.append(f"{ct.product_name} (không tìm thấy biến thể để hoàn kho)")
        if sl_giao <= 0:
            db.delete(ct)
        else:
            ct.quantity = sl_giao
    ...
    ghi_chu_thieu = ("Thiếu hàng: " + "; ".join(thieu_hang + loi_kho)) if (thieu_hang or loi_kho) else ""
    ghi_chu_manual = (ghi_chu_picker or "").strip()
    if ghi_chu_thieu and ghi_chu_manual:
        don.picker_note = f"{ghi_chu_thieu} | Ghi chú picker: {ghi_chu_manual}"
    elif ghi_chu_thieu:
    ...
    db.commit()
    return {"status": "success", "message": f"Đơn #{don.id} đã xác nhận hoàn thành", "partial": len(thieu_hang) > 0, "picker_note": don.picker_note, "delivered_total": tong_giao}
...
def xac_nhan_don(don_id: int, data: Optional[YeuCauXacNhanGiao] = None, db: Session = Depends(get_db)):
    try:
        ...
        if not don:
            raise HTTPException(status_code=404, detail="Hóa đơn không tồn tại")
        if don.status not in ("assigned", "accepted"):
...
@router.put("/orders/{don_id}/deliver")
def giao_hang_json(don_id: int, data: YeuCauGiaoHang, db: Session = Depends(get_db)):
    duong_dan_anh = parse_duong_dan_anh(data.photo_path)
    return _giao_hang_noi_bo(don_id, data.picker_id, duong_dan_anh, data.items, db, data.picker_note)
...
def _giao_hang_noi_bo(don_id: int, picker_id: int, photo_paths: list, items: list, db: Session, ghi_chu_picker: str = "") -> dict:
    if not photo_paths:
    ...
    ket_qua = _xac_nhan_giao_logic(don, items if items else None, db, ghi_chu_picker)
    don.delivered_by_id, don.delivered_at = picker.id, now_vn()
    ...
    if abs_paths:
        chu_thich = f"Đơn #{don.id} • {don.customer_name or 'Khách lẻ'}\nPicker: {picker.name}\n" + (don.delivered_at.strftime("%Y-%m-%d %H:%M") if don.delivered_at else "")
        if don.picker_note:
...
@anh_router.get("/delivery-proofs/pending")
def danh_sach_anh_chua_sync(since_order_id: int = 0, limit: int = 200, db: Session = Depends(get_db)):
    gioi_han = min(max(limit, 1), 500)
    don_hang = db.query(DonHang).filter(DonHang.status == "completed", DonHang.delivery_photo_path.isnot(None), DonHang.delivery_photo_path != "", DonHang.id > since_order_id).order_by(DonHang.id.asc()).limit(gioi_han).all()
    data = []
    for don in don_hang:
        rels = [r for r in parse_duong_dan_anh(don.delivery_photo_path) if r and not r.startswith("local://")]
        if not rels:
            continue
        ten_files = [os.path.basename(r) for r in rels]
        download_urls = [r if r.startswith("/delivery-proofs/") else f"/delivery-proofs/{os.path.basename(r)}" for r in rels]
        data.append({"order_id": don.id, "delivered_at": don.delivered_at.strftime("%Y-%m-%d %H:%M") if don.delivered_at else "", "customer_name": don.customer_name or "Khách lẻ", "photo_path": rels[0], "photo_paths": rels, "file_name": ten_files[0] if ten_files else "", "file_names": ten_files, "download_url": download_urls[0] if download_urls else "", "download_urls": download_urls})
...
def xac_nhan_anh_local(data: XacNhanLocalAnh, db: Session = Depends(get_db)):
    ...
    ten_files_local = [os.path.basename(str(x).strip()) for x in (data.local_file_names or []) if str(x).strip()] or [os.path.basename(p) for p in current_paths if str(p).strip()]
    local_paths = [f"local://delivery_proofs/{ten}" for ten in ten_files_local if ten]
    if local_paths:
