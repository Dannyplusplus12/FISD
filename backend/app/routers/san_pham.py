...
from ..models import SanPham, BienThe
from ..schemas.san_pham import TaoSanPham, CapNhatSanPham
from ..utils import now_vn
from ..config import settings

router = APIRouter(prefix="/products", tags=["Sản phẩm"])

_EXTENSIONS_HOP_LE_ANH = {".jpg", ".jpeg", ".png", ".webp", ".heic", ".bmp"}
...
def _luu_anh_san_pham(file: UploadFile) -> str:
    ...
    if ext not in _EXTENSIONS_HOP_LE_ANH:
...
def lay_danh_sach(search: str = "", db: Session = Depends(get_db)):
    ...
    for sp in san_pham:
        ...
        khoang_gia = "Hết hàng"
        if gia_list:
            min_g, max_g = min(gia_list), max(gia_list)
            khoang_gia = f"{min_g:,} - {max_g:,}" if min_g != max_g else f"{min_g:,}"
        ket_qua.append({
...
def upload_anh(file: UploadFile = File(...)):
    duong_dan = _luu_anh_san_pham(file)
